...
class CR(nn.Module):
    ...
    def export_to_onnx(self, outfile):
        print('Exporting to onnx')
        # buffer_size random values in 1 sequence element in 1 sequence
        inputs = torch.rand(1, 1, frame_size)
        h0 = torch.zeros(3, 1, self.rnn_hidden_size)

        _out, _hn = self(inputs, h0)

        input_names=['inputs', 'h0']
        output_names=['output', 'hn']

        torch.onnx.export(self, (inputs, h0), outfile, input_names, output_names)
